          ...
          {tab === 'live' && (
                  <div style={{ padding:'0 16px' }}>
                              <div style={{ display:'flex', gap:7, overflowX:'auto', padding:'12px 0', WebkitOverflowScrolling:'touch', scrollbarWidth:'none' }}>
                                {CATS.map(c => (
                                  <div key={c} style={{ ...S.chip, ...(cat===c ? S.chipOn : {}), whiteSpace:'nowrap', flexShrink:0 }} onClick={() => setCat(c)}>{c}</div>
                                ))}
                              </div>
                    {loading && <div style={S.empty}>加载中…</div>}
                    {!loading && pledges.length === 0 && <div style={S.empty}>还没有公开誓言，成为第一个！</div>}
                    {pledges.map(p => {
                                ...
                                return (
                                                <div key={p.id} style={S.pledgeCard} onClick={() => nav(`/pledge/${p.id}`)}>
                                                                  <div style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:10 }}>
                                                                                      <div style={{ ...S.ava, background:'#C8922A' }}>{(p.profiles?.nickname || '？')[0]}</div>
                                                                                      <div style={{ flex:1, minWidth:0 }}>
                                                                                                            <div style={{ fontSize:14, fontWeight:700, fontFamily:'Noto Serif SC,serif', marginBottom:3, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.title}</div>
                                                                                                            <div style={{ fontSize:11, color:'#9A8A70' }}>{p.profiles?.nickname ?? '匿名'} · {PERIOD_LABEL[p.period]}度誓言 · 押{p.stake_coins}金币</div>
                                                                                        </div>
                                                                                      <div style={{ ...S.tag, background:'#FDF3E0', color:'#7A5A18', flexShrink:0 }}>进行中</div>
                                                                  </div>
                                                                ...
                                                                <div style={{ fontSize:11, color:'#B8A88A' }}>还剩{daysLeft}天结束</div>
                                                </div>
                                              )
                    })}
                  </div>
          ...
          {tab === 'done' && (
                  <div style={{ padding:'0 16px' }}>
                            <div style={S.mentorCard}>
                                        <div style={{ position:'absolute', right:-10, top:-10, fontSize:80, opacity:.07, lineHeight:1, pointerEvents:'none' }}>🏆</div>
                                        <div style={{ fontSize:11, color:'rgba(255,255,255,.5)', marginBottom:8, fontWeight:600, letterSpacing:.5 }}>本周导师推荐</div>
                                        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                                                      <div style={{ ...S.ava, width:48, height:48, fontSize:20, background:'linear-gradient(135deg,#C8922A,#E8B84A)', flexShrink:0 }}>铁</div>
                                                      <div>
                                                                      <div style={{ fontSize:15, fontWeight:700, color:'#fff', fontFamily:'Noto Serif SC,serif' }}>铁汉</div>
                                                                      <div style={{ fontSize:11, color:'rgba(255,255,255,.5)', marginTop:2 }}>健身增肌 · 完成4次 · 帮助了83人</div>
                                                      </div>
                                                      <div style={{ marginLeft:'auto', textAlign:'center', flexShrink:0 }}>
                                                                      <div style={{ fontSize:18, fontWeight:700, color:'#E8B84A' }}>4.9</div>
                                                                      <div style={{ fontSize:10, color:'rgba(255,255,255,.4)' }}>导师评分</div>
                                                      </div>
                                        </div>
                                        ...
                                        <div style={{ display:'flex', gap:8 }}>
                                                      <button style={S.smBtnLight}>查看主页</button>
                                                      <button style={S.smBtnGhost}>向他提问 · 20金币</button>
                                        </div>
                            </div>
                            <div style={{ fontSize:11, color:'#9A8A70', fontWeight:600, marginBottom:10, letterSpacing:.5 }}>🎖 近期成功经验分享</div>
                    {SUCCESS_STORIES.map((s, idx) => (
                                <div key={idx} style={S.successCard}>
                                              <div style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:10 }}>
                                                              <div style={{ ...S.ava, background:s.color, flexShrink:0 }}>{s.ava}</div>
                                                              <div style={{ flex:1, minWidth:0 }}>
                                              ...
                                              <div style={{ display:'flex', alignItems:'center', gap:8, borderTop:'0.5px solid #F0EAE0', paddingTop:10 }}>
                                                              ...
                                                              <button style={S.actionBtn}>❓ 提问</button>
                                                              <div style={{ marginLeft:'auto', fontSize:11, color:'#C8922A', fontWeight:600 }}>+20金币/回答</div>
                                              </div>
                                </div>
                              ))}
                  </div>
