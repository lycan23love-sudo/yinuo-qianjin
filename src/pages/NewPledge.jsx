...
import { getPledgeCategory, inferPledgeCategory, inferPledgeTag } from '../lib/pledgeCategories'
...
export default function NewPledge() {
  ...
  const [form, setForm] = useState({ title: '', categoryKey: 'study', categoryTag: '读书学习', period: 'week', stake: 10, charity: '山区图书馆', verify: 'screenshot', isPublic: true })
  ...
  function choosePeriod(nextPeriod) {
    ...
    set('period', nextPeriod)
  }




  function confirmYearPeriod() {
  ...
  const currentCategory = hasTitle ? inferPledgeCategory({ title: form.title }) : getPledgeCategory(form.categoryKey)
  const currentTag = hasTitle
    ? inferPledgeTag({ title: form.title, category_key: currentCategory.key })
    : currentCategory.tags[0]
  ...
  async function handleSubmit() {
    ...
    setLoading(true)
    try {
      const pledge = await createPledge(session.user.id, { title: form.title, categoryKey: currentCategory.key, categoryLabel: currentCategory.label, categoryTag: currentTag, period: form.period, stakeCoins: form.stake, charityTarget: form.charity, verifyType: form.verify, isPublic: form.isPublic })
      refreshProfile()
    ...
    <div className="contractPage">
      ...
      <div className="contractBody">
        <section className={'scrollStage ' + (opened ? 'isOpen' : '')}>
          ...
          {!opened && <div className="closedSeal"><button className="openBtn" onClick={() => setOpened(true)}>启誓</button></div>}
        </section>
        <section className="termsPanel"><div className="termsHead"><h2>契约条款</h2><span>{currentCoins.toLocaleString()} 金币可用</span></div>
          <div className="termGroup autoCategoryGroup"><div className="termLabel">系统归类</div><div className="autoCategoryBox"><span className="autoCategoryPill">{currentCategory.emoji} {currentCategory.label}</span><span className="autoCategoryText">{hasTitle ? `${currentTag} · 用于指数大厅与同行推荐` : '写下誓言后自动识别，用于指数大厅与同行推荐'}</span></div></div>
          <div className="termGroup"><div className="termLabel">期限</div><div className="periodTabs">{PERIODS.map(p => <button key={p.key} className={'periodTab ' + (form.period === p.key ? 'active' : '')} onClick={() => choosePeriod(p.key)}><span>{p.label}</span>{p.key === 'week' && <em>推荐·初修</em>}</button>)}</div></div>
        ...
        <section className="ritualPanel"><p>{form.stake} 金币将在立约后托管。若失败，捐给「{form.charity}」。</p><button className="stampBtn" onClick={stamp} disabled={loading || sealing}>{!opened ? '展开画轴' : sealed ? '印已落，诺已成' : sealing ? '落印中...' : '压上守诺印'}</button><button className="submitBtn" onClick={handleSubmit} disabled={!sealed || loading || !profile || noCoins}>{!profile ? '加载账户中...' : loading ? '立约中...' : '正式立约，托管 ' + form.stake + ' 金币'}</button></section>{showYearConfirm && <div className="confirmLayer"><div className="confirmDialog"><div className="confirmKicker">三百六十五日令</div><h3>长路漫漫，容易断诺。</h3><p>创世行者，你确定一上来就要开启最高规格的‘终身斋戒’吗？</p><div className="confirmActions"><button className="confirmSecondary" onClick={chooseGentlePeriod}>量力而行</button><button className="confirmPrimary" onClick={confirmYearPeriod}>确定</button></div></div></div>}
      </div>
    </div>
  )
}
