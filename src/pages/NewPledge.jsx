...
export default function NewPledge() {
  ...
  const [form, setForm] = useState({ title: '', categoryKey: 'study', categoryTag: '读书', period: 'week', stake: 10, charity: '山区图书馆', verify: 'screenshot', isPublic: true })
    ...
    <div className="contractPage">
      ...
      <div className="contractBody">
        ...
        <section className="termsPanel"><div className="termsHead"><h2>契约条款</h2><span>{currentCoins.toLocaleString()} 金币可用</span></div>
          <div className="termGroup"><div className="termLabel">誓言分类</div><div className="categoryTabs">{PLEDGE_CATEGORIES.map(c => <button key={c.key} className={'categoryTab ' + (form.categoryKey === c.key ? 'active' : '')} onClick={() => set('categoryKey', c.key)}><span>{c.emoji}</span>{c.label}</button>)}</div></div>
          <div className="termGroup"><div className="termLabel">具体行动</div><div className="tagGrid compactTags">{currentCategory.tags.map(t => <button key={t} className={'tagBtn ' + (form.categoryTag === t ? 'active' : '')} onClick={() => set('categoryTag', t)}>{t}</button>)}</div></div>
